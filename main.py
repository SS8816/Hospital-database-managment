...
app.config.update(
    ...
    MAIL_PORT='465',
    MAIL_USE_SSL=True,
    MAIL_USERNAME="2022ucp1949@mnit.ac.in",
    MAIL_PASSWORD="Sh@270205"
)
mail = Mail(app)
...
def patient():
    ...
    if request.method == "POST":
        ...
        if len(number) != 10:
            flash("Please provide a 10-digit number", "danger")
            return render_template('patient.html', doct=doct)

        query = Patients(
            email=email, name=name, gender=gender, slot=slot, disease=disease,
        ...
        msg = Message("Booking Confirmation", sender="your@gmail.com", recipients=[email])
        msg.body = f"YOUR BOOKING IS CONFIRMED. THANKS {email} FOR CHOOSING US!\nYour Entered Details are:\nName: {name}\nSlot: {slot}\nAppointment Time: {time}\nAppointment Date: {date}\n"
        mail.send(msg)
